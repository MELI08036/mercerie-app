...
[data-testid="stSidebar"] hr { border-color: var(--or) !important; opacity: 0.4; }
...
.stButton > button:hover {
    ...
    box-shadow: 0 6px 18px rgba(125,32,53,0.35);
}

/* ── Inputs ── */
.stTextInput input, .stNumberInput input, .stSelectbox select {
    border-radius: 8px !important;
    border: 1.5px solid var(--or-light) !important;
...
.hero-header p  { color: var(--or-light) !important; margin: 0 !important; font-size: 0.95rem; letter-spacing: 0.05em; }
...
/* Masquer le menu Streamlit sans casser le bouton sidebar mobile */
#MainMenu { visibility: hidden; }
footer    { visibility: hidden; }

/* Cacher uniquement le titre "app.py" dans le header, pas le header entier */
header [data-testid="stToolbar"]          { visibility: hidden; }
header [data-testid="stDecoration"]       { display: none; }
header [data-testid="stStatusWidget"]     { visibility: hidden; }

/* ⚠️ On ne touche PAS à header lui-même pour garder le bouton ☰ sur mobile */
</style>
""", unsafe_allow_html=True)


# ─────────────────────────────────────────────
#  DONNÉES — lecture / écriture CSV
# ─────────────────────────────────────────────
CATALOGUE_FILE = "catalogue.csv"
VENTES_FILE    = "ventes.csv"

CAT_COLS   = ["id", "nom", "categorie", "prix_unitaire", "stock_initial", "stock_actuel", "unite", "photo_base64", "date_ajout"]
VENTE_COLS = ["id", "date", "heure", "article_id", "article_nom", "categorie", "quantite", "prix_unitaire", "total", "vendeur"]
...
def charger_catalogue():
    if os.path.exists(CATALOGUE_FILE):
        try:
            df = pd.read_csv(CATALOGUE_FILE, dtype=str)
...
def prochain_id(df, prefix="A"):
    ...
    nums = []
    for i in ids:
        try: nums.append(int(str(i).replace(prefix, "")))
        except: pass
...
with st.sidebar:
    ...
    ruptures = cat_df[cat_df["stock_actuel"] <= 0] if not cat_df.empty else pd.DataFrame()
    faibles  = cat_df[(cat_df["stock_actuel"] > 0) & (cat_df["stock_actuel"] <= 5)] if not cat_df.empty else pd.DataFrame()
    if len(ruptures) > 0:
        st.markdown(f"🔴 **{len(ruptures)} rupture(s)** de stock !")
    if len(faibles) > 0:
        st.markdown(f"🟡 **{len(faibles)} article(s)** en stock faible")

    st.markdown(f"<div style='font-size:0.72rem;color:rgba(250,247,242,0.45);margin-top:30px;text-align:center'>{datetime.now().strftime('%d/%m/%Y · %H:%M')}</div>", unsafe_allow_html=True)
...
if "🏠" in page:
        ...
        if not vente_df.empty and "date" in vente_df.columns:
            try:
                ...
                fig.add_trace(go.Bar(
                    x=ca_par_jour["Date"], y=ca_par_jour["CA (FCFA)"],
                    name="CA Journalier", marker_color="#C9A84C", opacity=0.75
                ))
                fig.add_trace(go.Scatter(
                    x=ca_par_jour["Date"], y=ca_par_jour["CA cumulé"],
                ...
                st.info("Pas encore de données de ventes pour le graphique.")
        else:
    ...
    with col_d:
        ...
        if not cat_df.empty:
            ...
            for _, r in faibles.iterrows():
                ...
                <span style='color:#E8A020'>Stock faible · {r['stock_actuel']} {r.get('unite','')}</span></div>""",
                unsafe_allow_html=True)
        else:
    ...
    if not vente_df.empty:
        ...
        dern = vente_df.tail(8).iloc[::-1][["date", "heure", "article_nom", "categorie", "quantite", "total", "vendeur"]]
        dern.columns = ["Date", "Heure", "Article", "Catégorie", "Qté", "Total (FCFA)", "Vendeur"]
...
# ─────────────────────────────────────────────
            ...
            if st.button("💾  Enregistrer l'article"):
                ...
                if erreurs:
                    ...
                    if not cat_df.empty and nom.strip().lower() in cat_df["nom"].str.lower().tolist():
                        ...
                        cat_df = pd.concat([cat_df, pd.DataFrame([nouvel])], ignore_index=True)
                        sauver_catalogue(cat_df)
                        st.markdown(f"""<div class="msg-succes">
                        ✅ Article <b>{nom.strip()}</b> ajouté avec succès au catalogue !<br>
    ...
    with tab2:
        ...
        if cat_df.empty:
                            ...
                            with st.container():
                                ...
                                <div style='background:white;border-radius:10px;padding:12px 14px;margin-bottom:8px;
                                    ...
                                    <div style='font-family:"Playfair Display",serif;font-size:1rem;font-weight:600;
                                                color:var(--brun);margin:4px 0'>{art['nom']}</div>
                                    <div style='font-size:1.1rem;font-weight:700;color:var(--bordeaux)'>{float(art['prix_unitaire']):,.0f} FCFA</div>
                                    <div style='margin-top:6px;font-size:0.82rem;color:{couleur_stock};font-weight:600'>
                                        📦 {label_stock}
                                    </div>
                                </div>
                                """, unsafe_allow_html=True)
            ...
            # Modifier le stock
            st.markdown("<div class='or-divider'></div>", unsafe_allow_html=True)
            st.markdown("### ✏️ Modifier le stock d'un article (réapprovisionnement)")
            col_m1, col_m2, col_m3 = st.columns([3, 2, 1])
            with col_m1:
                noms_art = cat_df["nom"].tolist()
                art_choisi = st.selectbox("Article à réapprovisionner", noms_art, key="reapro_art")
            with col_m2:
            ...
            with col_m3:
                ...
                if st.button("➕ Ajouter"):
                    try:
                        ...
                        if qte_ajout <= 0: raise ValueError()
                        idx = cat_df[cat_df["nom"] == art_choisi].index[0]
                        cat_df.at[idx, "stock_actuel"] = int(cat_df.at[idx, "stock_actuel"]) + qte_ajout
                        sauver_catalogue(cat_df)
                        st.success(f"✅ Stock de **{art_choisi}** mis à jour → {cat_df.at[idx,'stock_actuel']} {cat_df.at[idx,'unite']}")
                        st.rerun()
                    except:
                        st.error("⚠️ Entrez un nombre entier positif pour la quantité à ajouter.")
        ...
        if not articles_dispo:
            ...
            with col_g:
                    ...
                    if qte_val > 0:
                        ...
                        <div style='background:linear-gradient(135deg,#FAF7F2,#F0EBE0);
                            ...
                            <div style='font-family:"Playfair Display",serif;font-size:2.2rem;
                                        font-weight:700;color:var(--bordeaux)'>{total_preview:,.0f} FCFA</div>
                        </div>""", unsafe_allow_html=True)
                except:
                    if qte_raw.strip():
                ...
                if st.button("✅  Valider la vente & Générer le Reçu"):
                    ...
                    if not qte_ok:
                        erreurs.append("Entrez une quantité valide.")
                    else:
                        try:
                    ...
                    if erreurs:
                        ...
                        vente_df = pd.concat([vente_df, pd.DataFrame([nouvelle_vente])], ignore_index=True)
                        sauver_ventes(vente_df)

                        # Mettre à jour le stock
                        idx_cat = cat_df[cat_df["nom"] == art_nom].index[0]
                        cat_df.at[idx_cat, "stock_actuel"] = stock_act - qte_final
                        sauver_catalogue(cat_df)
            ...
            with col_d:
                ...
                if "last_vente" in st.session_state:
                    ...
                    st.markdown(f"""
                    <div class="recu-box">
                        <div style='text-align:center;font-family:"Playfair Display",serif;font-size:1.1rem;margin-bottom:8px'>
                            🧵 MERCERIE PRO
                        </div>
                        <div style='text-align:center;font-size:0.75rem;margin-bottom:12px'>
                            ——————————————————
                        </div>
                        Date &nbsp;&nbsp;: {v['date']}<br>
                        Heure &nbsp;: {v['heure']}<br>
                    ...
                    <div style='background:white;border-radius:10px;padding:24px;text-align:center;
                        ...
                        <div style='margin-top:8px;font-size:0.9rem'>
                            Le reçu de la dernière vente<br>apparaîtra ici
                        </div>
                    </div>""", unsafe_allow_html=True)
            ...
            vente_df_fresh = charger_ventes()
            auj = date.today().isoformat()
            if not vente_df_fresh.empty and "date" in vente_df_fresh.columns:
                ventes_auj = vente_df_fresh[vente_df_fresh["date"] == auj]
                if ventes_auj.empty:
                    ...
                    affich = ventes_auj[["id", "heure", "article_nom", "quantite", "prix_unitaire", "total", "vendeur"]].copy()
                    affich.columns = ["Réf.", "Heure", "Article", "Qté", "PU (FCFA)", "Total (FCFA)", "Vendeur"]
...
# ─────────────────────────────────────────────
    ...
    if vente_df.empty:
        st.info("📊 Les statistiques apparaîtront dès vos premières ventes. Commencez à utiliser la caisse !")
    else:
        # ── Filtre période ──
        ...
        if vf.empty:
            ...
            with c1:
                ...
                <div class="kpi-value">{ca_periode:,.0f}</div><div class="kpi-sub">FCFA</div></div>""",
                unsafe_allow_html=True)
            with c2:
                st.markdown(f"""<div class="kpi-card" style='border-left-color:#7D2035'>
                <div class="kpi-title">Transactions</div>
                <div class="kpi-value">{nb_transactions}</div><div class="kpi-sub">ventes enregistrées</div></div>""",
                unsafe_allow_html=True)
            with c3:
                st.markdown(f"""<div class="kpi-card" style='border-left-color:#2D6A4F'>
                <div class="kpi-title">Panier Moyen</div>
                <div class="kpi-value">{panier_moyen:,.0f}</div><div class="kpi-sub">FCFA / vente</div></div>""",
                unsafe_allow_html=True)
            with c4:
                st.markdown(f"""<div class="kpi-card" style='border-left-color:#5C3D2E'>
                <div class="kpi-title">Top Article</div>
                <div class="kpi-value" style='font-size:1rem;line-height:1.3'>{art_top}</div>
                <div class="kpi-sub">+ vendu en qté</div></div>""",
                unsafe_allow_html=True)
            ...
            with col_a:
                st.markdown("#### 🏆 Articles les Plus Vendus (en quantité)")
            ...
            with col_b:
                ...
                palette = ["#C9A84C", "#7D2035", "#3D2B1F", "#2D6A4F", "#5C3D2E",
                           "#A64455", "#E8D5A3", "#8A8078"]
            ...
            if not cat_df.empty:
                st.markdown("<div class='or-divider'></div>", unsafe_allow_html=True)
                st.markdown("#### 📦 État des Stocks — Vue d'ensemble")

                col_s1, col_s2 = st.columns(2)

                with col_s1:
                    stock_df = cat_df[["nom", "stock_initial", "stock_actuel", "categorie"]].copy()
                    stock_df["vendu"] = stock_df["stock_initial"] - stock_df["stock_actuel"]
                    stock_df["vendu"] = stock_df["vendu"].clip(lower=0)
                    stock_top = stock_df.sort_values("vendu", ascending=False).head(10)

                    fig5 = go.Figure()
                    fig5.add_trace(go.Bar(name="Vendu", x=stock_top["nom"], y=stock_top["vendu"],
                                         marker_color="#7D2035"))
                    fig5.add_trace(go.Bar(name="Stock restant", x=stock_top["nom"], y=stock_top["stock_actuel"],
                                         marker_color="#C9A84C"))
                    fig5.update_layout(
                ...
                with col_s2:
                    st.markdown("##### Tableau récapitulatif des stocks")
                    tbl = cat_df[["nom", "categorie", "stock_actuel", "unite", "prix_unitaire"]].copy()
                    tbl["Valeur stock (FCFA)"] = tbl["stock_actuel"] * tbl["prix_unitaire"]
                    tbl["Statut"] = tbl["stock_actuel"].apply(
                        lambda x: "🔴 RUPTURE" if x <= 0 else ("🟡 Faible" if x <= 5 else "🟢 OK")
                    )
                    tbl = tbl[["nom", "categorie", "stock_actuel", "unite", "Valeur stock (FCFA)", "Statut"]]
                    tbl.columns = ["Article", "Catégorie", "Stock", "Unité", "Valeur (FCFA)", "Statut"]
                    tbl = tbl.sort_values("Stock")
                ...
                st.markdown(f"""
                <div style='background:linear-gradient(135deg,var(--brun),#5C3D2E);
                    ...
                    <div style='font-family:"Playfair Display",serif;color:var(--or);font-size:2rem;font-weight:700'>
                        {valeur_totale:,.0f} FCFA
                    </div>
                </div>""", unsafe_allow_html=True)

            # ── Tableau données brutes ──
            st.markdown("<div class='or-divider'></div>", unsafe_allow_html=True)
            with st.expander("📄 Données brutes des ventes (période sélectionnée)"):
                st.dataframe(vf, use_container_width=True, hide_index=True)
                csv_export = vf.to_csv(index=False).encode("utf-8")
                st.download_button("⬇️ Télécharger en CSV", csv_export, "ventes_export.csv", "text/csv")
